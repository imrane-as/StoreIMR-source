...
export default async function ProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (rows[0]) product = mapProduct(rows[0]) as typeof product;
  } catch {}
  if (!product) notFound();
  const mainImage = product.imageUrls?.[0] || "/storeimr-hero.png";
  ...
  return <main className="product-page">
    ...
    <section className="product-detail">
      <div><div className="product-detail-image" style={{backgroundImage:`url(${mainImage})`,backgroundPosition:product.position}}><span>{product.condition}</span></div>{product.imageUrls?.length>1&&<div className="product-gallery">{product.imageUrls.map((url,i)=><img src={url} alt={`${product.name} — photo ${i+1}`} key={url}/>)}</div>}</div>
